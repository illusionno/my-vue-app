...
onMounted(() => {
    if (origin.indexOf("https") === -1) {
        ElMessage.warning("请在 https 环境中使用!");
    }
    canvasWidth.value = window.screen.availWidth
    canvasHeight.value = window.screen.availHeight;
    nextTick(() => {
        onInit()
    })
})
const onInit = () => {
    ...
    document.querySelector(".canvasBox")?.append(canvas.value);

    document.querySelector(".canvasBox")?.append(video.value);
...
const tick = () => {
    if (video.value.readyState === video.value.HAVE_ENOUGH_DATA) {
        ...
        canvas.value.height = canvasHeight.value;
        if (!canvas2d.value) {
            canvas2d.value = canvas.value.getContext("2d");
        }
